...
Pick a range (7, 30 or 90 days), filter by account, and read the per-provider split, the chart and a breakdown table you can switch between model and day. Prices come from that list, with a copy bundled in the app for when it cannot be reached. Models no price list covers are counted in tokens and called out instead of being silently treated as free. You can price them yourself, or correct any listed price, with `%LOCALAPPDATA%\costats\pricing.json`, a flat map of model id to USD per million tokens that always wins:
