...
public sealed class StartupUpdateCoordinator
{
    ...
    /// <summary>
    /// The updater script sets this on a process it relaunches after a swap it
    /// could not complete. That process must not immediately try to apply the
    /// same pending update again: doing so is what spawned overlapping updater
    /// runs that kept the install directory locked and the update stuck in a
    /// loop. A genuine, user-initiated launch starts with a clean environment,
    /// so the deferred update applies then instead.
    /// </summary>
    internal const string DeferredRelaunchVariable = "COSTATS_UPDATE_DEFERRED";
    ...
    public async Task<bool> TryApplyPendingUpdateAsync(CancellationToken cancellationToken, bool manualTrigger)
    {
        ...
        // A process the updater relaunched after a deferred swap must not
        // re-apply automatically, or it recreates the relaunch loop. A user's
        // deliberate "install now" click (manualTrigger) is still honoured.
        if (!manualTrigger &&
            string.Equals(Environment.GetEnvironmentVariable(DeferredRelaunchVariable), "1", StringComparison.Ordinal))
        {
            Trace.WriteLine("[costats-update] skipping auto-apply: this process was relaunched by a deferred updater run");
            return false;
        }
    ...
    internal const string UpdaterScriptContents = """
param(
